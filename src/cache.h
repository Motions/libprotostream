...
public:
    ...
    offset_t link_at(offset_t offset, unsigned level) {
        ...
        const auto it = offsets.find(header.kf_num + (1 << level));

        if (it != offsets.end()) {
            return it->second;
        }

        const auto ptr = backend.read(offset + reduced_keyframe_header::skiplist_offset(),
                                      sizeof(offset_t) * reduced_keyframe_header::skiplist_height);

        const auto skiplist = reinterpret_cast<const offset_t *>(detail::as_ptr(ptr));

        for (unsigned i = 0; i < reduced_keyframe_header::skiplist_height; ++i) {
            const auto offset = detail::betoh<offset_t>(skiplist[i]);

            if (offset != no_keyframe) {
                offsets.emplace(header.kf_num + (1 << i), offset);
            }
        }
