...
constexpr unsigned skiplist_height = 10;


constexpr char magic[] = "PROTOSTR";
constexpr size_t magic_size = sizeof(magic) - 1;    //without null byte
static_assert(magic_size == 8, "magic size");     //check for consistency with spec.txt

constexpr ssize_t header_size = magic_size + sizeof(offset_t) * 4 /* pointers */ +
                                sizeof(uint32_t) /* frames/kfr */ + 4 /* reserved */;
constexpr size_t kf_header_size = sizeof(offset_t) * (2 + skiplist_height) + sizeof(uint32_t);
