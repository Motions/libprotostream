...
public:
    ...
    proto_header_type get_proto_header() const {
        const auto size = header.get<fields::kf0_offset>() - header.get<fields::proto_header_offset>();
        return proto_header_factory_type::build(backend.read(header.get<fields::proto_header_offset>(), size), size);
    }
    ...
    public:
        ...
        delta_iterator end() const {
            return {str, 0, std::min(str.header_field<fields::frame_count>(), (id() + 1) * str.header_field<fields::frames_per_kf>())};
        }
    ...
    public:
        ...
        keyframe_iterator &operator+=(keyframe_id_t diff) {
            ...
            if (auto offset_opt = data.str.cache.offset_of(data.id() + diff)) {
                ...
                for (unsigned level = fields::skiplist_height - 1; diff > 0; level--) {
                    while (diff >= (1u << level)) {
                        diff -= 1u << level;
    ...
    keyframe_iterator begin() const {
        return {*this, header.get<fields::kf0_offset>()};
    }
    ...
    void append_delta(std::uint8_t *data, delta_size_t size) {
        assert(header.get<fields::frame_count>() % header.get<fields::frames_per_kf>() != 0);

        const auto offset = backend.size();
        backend.write_num(offset, size);
        backend.write(offset + sizeof(delta_size_t), size, data);

        header.get<fields::frame_count>()++;
        header.get<fields::file_size>() += sizeof(delta_size_t) + size;
    ...
    void append_keyframe(std::uint8_t *data, std::size_t size) {
        ...
        const auto offset = backend.size();
        const auto id = header.get<fields::keyframe_count>();
        ...
        header.get<fields::frame_count>()++;
        header.get<fields::keyframe_count>()++;
        header.get<fields::file_size>() += reduced_keyframe_header::size + size;
...
stream<Args...>::stream(const char *path)
    ...
    if (header.get<fields::file_size>() != file_size) {
        throw std::runtime_error("File size not consistent with data in header");
    }

    if (header.get<fields::proto_header_offset>() > file_size - file_header::size) {
        throw std::runtime_error("Invalid proto header offset");
    }

    if (header.get<fields::kf0_offset>() > file_size - file_header::size) {
        throw std::runtime_error("Invalid keyframe 0 offset");
    }

    if (header.get<fields::proto_header_offset>() > header.get<fields::kf0_offset>()) {
...
stream<Args...>::stream(const char *path, std::uint32_t frames_per_kf, void *proto_header,
    ...
    header.get<fields::file_size>() = end;
    header.get<fields::kf0_offset>() = end;
    header.get<fields::proto_header_offset>() = file_header::size;
    header.get<fields::frames_per_kf>() = frames_per_kf;
    header.write(backend, 0);
