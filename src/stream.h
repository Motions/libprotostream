...
public:
    ...
    stream(const char* path, std::uint32_t frames_per_kf, const void* proto_header, std::size_t proto_header_size);
    ...
    proto_header_type get_proto_header() const {
        ...
        return proto_header_factory_type::build(backend.read(header_field<fields::proto_header_offset>(), size), size);
    }
