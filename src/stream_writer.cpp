...
int stream_writer::append_keyframe(const void *data, size_t data_size) {
    ...
    keyframe_offsets.insert({keyframe_count, current_pos});
    kf_cache.insert({keyframe_count, new_hdr});
    deltas_for_current = 0;
    offset_t tmp;
    //kf_num
    writebuf<offset_t>(&tmp, keyframe_count);
    ok &= write_loop(fd, &tmp, sizeof(offset_t));
    //delta_start
    writebuf<offset_t>(&tmp, new_hdr.deltas);
    ok &= write_loop(fd, &tmp, sizeof(offset_t));
    //empty skiplist
    ...
    //kf_size, TODO remove?
    uint32_t tmp2;
    writebuf<uint32_t>(&tmp2, new_hdr.data_size);
    ok &= write_loop(fd, &tmp2, sizeof(uint32_t));
    for(unsigned i = 0; i < skiplist_height; i++)
        forward_pointers.insert({keyframe_count + (1 << i),
                {current_pos, i}});
...
bool stream_writer::fixup_skiplist(offset_t new_kf_num, offset_t where) {
    ...
    offset_t where_buf;
    writebuf<offset_t>(&where_buf, where);
    bool ok = true;
...
int stream_writer::append_delta(const void *data, size_t data_size) {
    ...
    delta_size_t size;
    writebuf<delta_size_t>(&size, data_size);
