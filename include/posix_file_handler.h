...
public:
    using buffer_type = std::conditional_t<mode == file_mode_t::READ_ONLY, const std::uint8_t*, std::uint8_t*>;
...
template<file_mode_t mode>
auto posix_file_handler<mode>::mremap(const buffer_type buffer, std::size_t old_size, std::size_t new_size) -> buffer_type {
#ifdef HAVE_MREMAP
    if (!buffer) {
        assert(old_size == 0);
