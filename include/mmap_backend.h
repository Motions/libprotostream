...
public:
    ...
    mmap_backend(const char* path)
            : file{path}, used_size{file.size()},
              buffer{std::make_unique<mmap_guard<posix_file_handler<mode>>>(file)} { }
...
private:
    ...
    std::unique_ptr<mmap_guard<posix_file_handler<mode>>> buffer;
...
inline mmap_backend<file_mode_t::READ_APPEND>::~mmap_backend() noexcept(false) {
    auto file_size = buffer->size();

    /* Release the mmaped memory before truncating the file */
    buffer.release();
