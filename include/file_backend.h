...
/** A low-level file backend (CRTP base for mmap_backend and file_backend).
 *
...
struct file_backend {
    ...
    /** Writes `value` into the file starting from `offset` (converting it previously to the big-endian byte order) */
    template<class T>
