...
namespace {
constexpr const char serialised[] = "\x01\x23\x45\x67\x76\x54\x32\x10" /* keyframe number */
...
static_assert(protostream::reduced_keyframe_header::size == sizeof(serialised) - 1,
              "Wrong header size");

TEST(reduced_keyframe_header, read) {
    const auto header = protostream::reduced_keyframe_header::read(
        reinterpret_cast<const std::uint8_t*>(serialised));
