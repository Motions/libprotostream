...
using pairs = product<streams::read_streams, simple_tests::tests>::type<testing::Types>;
...
TYPED_TEST(integration_read_simple, deltas) {
    ...
    for(const auto &keyframe : *this->stream) {
        if (cnt + TypeParam::test::frames_per_keyframe <= TypeParam::test::frame_count) {
            ...
            ASSERT_EQ(TypeParam::test::frame_count - cnt, std::distance(keyframe.begin(), keyframe.end()));
        }
