...
template<
        template<class...> class Wrapper1,
        template<class...> class Wrapper2,
        class... Args1,
        class... Args2>
struct product<Wrapper1<Args1...>, Wrapper2<Args2...>> {
private:
    template<template<class...> class Wrapper, class Arg1>
    using aux = Wrapper<wrapper<Arg1, Args2>...>;
public:
    template<template<class...> class Wrapper>
    using type = typename concat<aux<std::tuple, Args1>...>::template type<Wrapper>;
};
